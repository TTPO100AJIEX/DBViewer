export default function tableLayoutQuerySrc(type = "id")
{
    let condition = "";
    switch (type)
    {
        case "name": { condition = "pg_class.relname = %L"; break; }
        default: { condition = "pg_class.oid = %L"; }
    }

    return `WITH enums (enumtypid, enumlabels) AS ( SELECT enumtypid, array_agg(enumlabel::text) AS enumlabels FROM pg_enum GROUP BY enumtypid )
    SELECT
    ...
    WHERE ${condition} AND pg_attribute.attnum > 0 AND NOT pg_attribute.attisdropped
